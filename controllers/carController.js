  ...
  try {
    ...
    if (req.files) {
      for (const file of req.files) {
        const imageUrl = await imageUploadCloudinary(file.path);
    ...
    const newCar = new Car({
      ...
      bookedTimeSlots, rentPerHour, type, location, images: imageUrls
    });
...
export const updateCar = async (req, res, next) => {
  try {
      ...
      if (req.files && req.files.length > 0) {
          ...
          const uploadPromises = req.files.map(file => imageUploadCloudinary(file.path));
          imageFiles = await Promise.all(uploadPromises);
      } else {
          // If no new files, keep existing images
          imageFiles = car.images;
      }
